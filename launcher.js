...
//
// This is the container's PID 1 (`npm start` runs it). It spawns index.js and
// watches its exit code. When index.js exits with code 75 it is asking to be
// updated/restarted: the launcher applies the staged build (overlaying files
// while the app is NOT running, so nothing replaces itself in place) and
// relaunches. Any other exit code stops the launcher too, so a genuine crash or
// a clean shutdown still propagates out to Docker. Run `npm run start:direct`
// to bypass the launcher (self-apply path; needs an external restart policy).
...
console.log("[launcher] starting MusicD LMS Remote (auto-update enabled)");
start();
