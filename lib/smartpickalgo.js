...
const MAX_STRETCH_GENRES = 5;
const MAX_STRETCH_ROSTER = 15;
// How many MusicBrainz tag spellings to try for one library genre. Each is a
// rate-gated request, so this is a budget, not a wish list.
 ...
 *
 * The share line alone was not enough and the stretch pick went missing
 * because of it: a library with a FLAT genre spread — say 300 albums over
 * eight genres, the smallest holding 12 — has nothing at or under 2%, so this
 * returned [] and the day's sixth pick simply never got built. "Barely
 * touches" is a statement about THIS library, not an absolute fraction, so
 * when nothing clears the line the rarest genres stand in. The single
 * most-owned genre is never among them: that one is the library.
 */
function smartStretchGenres(weights, totalAlbums, share) {
  const total = totalAlbums || 0;
  if (!total) return [];
  const all = [];
  for (const [genre, albums] of weights) if (genre) all.push({ genre, albums });
  // One genre is the whole library — there is no "outside" to reach for.
  if (all.length < 2) return [];
  all.sort((a, b) => a.albums - b.albums ||
    (a.genre < b.genre ? -1 : a.genre > b.genre ? 1 : 0));
  const limit = share == null ? STRETCH_SHARE : share;
  const under = all.filter(g => (g.albums / total) <= limit);
  return under.length ? under : all.slice(0, all.length - 1);
}

/* One library genre → the tag spellings worth asking MusicBrainz for.
 *
 * The second reason the stretch pick went missing. A genre string comes from
 * the file's own GENRE tag and is routinely a COMPOUND — "Alternative & Punk",
 * "Hip-Hop/Rap", "Folk, World, & Country" are all ordinary iTunes/Discogs
 * spellings. MusicBrainz tags are atomic and lower case ("punk", "hip hop",
 * "folk"), so a single `tag:"alternative & punk"` matches nothing at all and
 * the roster comes back empty however good the rest of the pipeline is.
 *
 * Order is most-specific-first, because a hit on the whole string is a better
 * answer than a hit on half of it, and the caller stops at the first roster it
 * gets. The `&` → ` and ` form comes second so "Drum & Bass" is tried whole
 * ("drum and bass" is the real tag) before it is split into "drum" and "bass",
 * which are not genres and would hand back a junk roster.
 */
const GENRE_SPLIT_RE = /\s*(?:&|\/|\+|,|;)\s*/;

function genreTagCandidates(genre) {
  const raw = String(genre || "").trim();
  if (!raw) return [];
  const out = [];
  const push = (s) => {
    const v = String(s || "").toLowerCase().replace(/\s+/g, " ").trim();
    // A one-character fragment ("r" out of "R&B") is noise, not a tag.
    if (v.length < 2) return;
    if (out.indexOf(v) === -1) out.push(v);
  };
  push(raw);
  if (raw.indexOf("&") !== -1) push(raw.replace(/&/g, " and "));
  for (const part of raw.split(GENRE_SPLIT_RE)) {
    push(part);
    if (part.indexOf("-") !== -1) push(part.replace(/-/g, " "));
  }
  if (raw.indexOf("-") !== -1) push(raw.replace(/-/g, " "));
  return out.slice(0, MAX_TAG_CANDIDATES);
}
...
module.exports = {
  smartPickSeeds, smartStretchGenres, genreTagCandidates, collectSmartCandidates,
  ...
  MAX_STRETCH_ROSTER, MAX_TAG_CANDIDATES, MAX_RESOLVES, PICK_COUNT,
};
