...
function mockResultFor(playerId, command) {
  ...
  if (cmd === "favorites") {
    if (command[1] === "items") {
      return { count: 1, loop_loop: [
        // want_url:1 was asked for, so a url comes back. Escaped the way LMS
        // stores it, to exercise the decoded comparison.
        { id: "0", name: "Kind of Blue", title: "Kind of Blue", url: "db:album.title=Kind%20of%20Blue&contributor.name=Miles%20Davis" },
        // A row with no url at all — LMS omits it for some item types, and it
        // must not become "undefined" in the set.
        { id: "1", name: "A folder", hasitems: 1 },
      ] };
    }
    return { count: 1 };
  }
...
(async () => {
  ...
  // ---- LMS's own Favorites (v1.0.83) ----
  //
  // Verified against Slim/Plugin/Favorites/Plugin.pm: every verb is
  // needs-client 0, add takes url+title, delete accepts url:, and items only
  // returns urls when want_url:1 is passed (and must NOT be sent menu:1).
  {
    const urls = await lms.favouriteUrls();
    assert.deepStrictEqual(lastReq.command, ["favorites", "items", 0, 999, "want_url:1"]);
    assert.strictEqual(lastReq.playerId, "", "favorites is server-global — never a player id");
    ok("favouriteUrls asks for want_url:1, server-global, without menu:1");
    assert.ok(!lastReq.command.includes("menu:1"), "menu:1 would strip db: urls");
    assert.ok(urls.has("db:album.title=Kind of Blue&contributor.name=Miles Davis"),
              "urls are compared DECODED");
    assert.strictEqual(urls.size, 1, "a row with no url is skipped, not stored as undefined");
    ok("favouriteUrls decodes and skips url-less rows");

    // Already present → no add is sent (the CLI `add` does not de-duplicate).
    const before = reqLog.length;
    const r1 = await lms.favouritesAdd("db:album.title=Kind%20of%20Blue&contributor.name=Miles%20Davis", "Kind of Blue");
    assert.strictEqual(r1, true);
    assert.strictEqual(lastReq.command[1], "items", "an existing favourite must not be added twice");
    assert.strictEqual(reqLog.length, before + 1, "only the read happened");
    ok("favouritesAdd skips an album LMS already has");

    await lms.favouritesAdd("db:album.title=Bloom&contributor.name=Larkin%20Poe", "Bloom");
    assert.deepStrictEqual(lastReq.command,
      ["favorites", "add", "url:db:album.title=Bloom&contributor.name=Larkin%20Poe", "title:Bloom"]);
    assert.strictEqual(lastReq.playerId, "");
    ok("favouritesAdd sends url: and title:, server-global");

    await lms.favouritesRemove("db:album.title=Bloom&contributor.name=Larkin%20Poe");
    assert.deepStrictEqual(lastReq.command,
      ["favorites", "delete", "url:db:album.title=Bloom&contributor.name=Larkin%20Poe"]);
    ok("favouritesRemove deletes by url, not by positional item_id");

    await assert.rejects(() => lms.favouritesAdd("", "T"), /url and a title/);
    await assert.rejects(() => lms.favouritesAdd("u", ""), /url and a title/);
    ok("favouritesAdd refuses a missing url/title rather than tripping LMS's socket close");
  }

  // The favourite URL an album is filed under.
  {
    const { favouriteUrl } = require("./lms");
    // The server's own answer wins over anything reconstructed.
    assert.strictEqual(favouriteUrl({ favUrl: "db:given", extid: "qobuz:album:1", title: "T", artist: "A" }),
                       "db:given");
    // Then the online-library extid (Slim::Schema::Album::url returns it).
    assert.strictEqual(favouriteUrl({ extid: "qobuz:album:1", title: "T", artist: "A" }), "qobuz:album:1");
    // Then the db: reconstruction, escaped the way LMS's BUNDLED URI::Escape
    // does — which is exactly encodeURIComponent. !*'() must stay literal; a
    // build using modern RFC-3986 URI::Escape would escape them and no
    // favourite would ever match.
    assert.strictEqual(favouriteUrl({ title: "Vol. 2 (Deluxe)", artist: "P!nk" }),
                       "db:album.title=Vol.%202%20(Deluxe)&contributor.name=P!nk");
    assert.strictEqual(favouriteUrl({ title: "Don't Look Back", artist: "Boston" }),
                       "db:album.title=Don't%20Look%20Back&contributor.name=Boston");
    // A symbol-only title still produces a usable address (v1.0.82's albums).
    assert.strictEqual(favouriteUrl({ title: "<|\u00b0_\u00b0|>", artist: "Caravan Palace" }),
                       "db:album.title=%3C%7C%C2%B0_%C2%B0%7C%3E&contributor.name=Caravan%20Palace");
    assert.strictEqual(favouriteUrl({ artist: "A" }), null, "no title, no address");
    assert.strictEqual(favouriteUrl({}), null);
    ok("favouriteUrl prefers the server's url, then extid, then a db: reconstruction");
  }
  ...
  // ---- coverKey: what a STREAMING row actually looks like ------------------
  //
  // These fixtures are the shapes a live LMS sends, which is the whole point.
  // The existing album fixture uses a raw absolute artwork_url — a shape
  // `titles` and `playlists tracks` never return — so every defect below
  // passed unnoticed.
  {
    const ck = _internal.coverKey;
    const b64 = (u) => "url-" + Buffer.from(u, "utf8").toString("base64url");

    // A local track: an 8-hex cover id, used as-is.
    assert.strictEqual(ck({ coverid: "a1b2c3d4" }), "a1b2c3d4");
    assert.strictEqual(ck({ artwork_track_id: 999 }), "999");
    ok("coverKey passes a local cover id through");

    // A REMOTE track. Slim::Schema::RemoteTrack sets id => -int($self) and
    // coverid => id, so tag `c` answers a NEGATIVE number. Asking LMS for
    // artwork under it does not 404 — Slim::Web::Graphics sees the leading "-"
    // and serves its radio-tower placeholder with HTTP 200, which is exactly
    // why Qobuz playlist mosaics filled with grey towers.
    assert.strictEqual(ck({ coverid: "-140351234" }), null);
    assert.strictEqual(ck({ coverid: -140351234 }), null);
    ok("coverKey refuses a remote track's negative pseudo cover id");

    // Tag K does not return the origin URL; it returns LMS's own proxy path.
    // Keeping it verbatim and re-wrapping it produced a double-proxied 404.
    assert.strictEqual(
      ck({ artwork_url: "/imageproxy/https%3A%2F%2Fstatic.qobuz.com%2Fcovers%2Fabc_600.jpg/image.jpg" }),
      b64("https://static.qobuz.com/covers/abc_600.jpg"));
    ok("coverKey unwraps an imageproxy path to the origin URL");

    // The real streaming case: negative id AND a proxied url on one row.
    assert.strictEqual(
      ck({ coverid: "-99", artwork_url: "/imageproxy/https%3A%2F%2Fx.test%2Fc.jpg/image.jpg" }),
      b64("https://x.test/c.jpg"));
    ok("coverKey falls through from a pseudo id to the artwork url");

    // A relative path we cannot unwrap is NO cover — minting a key for it would
    // resolve to a placeholder or a 404, both of which look like working art.
    assert.strictEqual(ck({ artwork_url: "html/images/radio.png" }), null);
    assert.strictEqual(ck({}), null);
    ok("coverKey reports no cover rather than minting an unusable key");
  }
