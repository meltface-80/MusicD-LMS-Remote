...
/* A fold that KEEPS punctuation and symbols, for titles that have nothing else.
 *
 * `norm()` strips everything outside [a-z0-9], which is right for matching
 * words — but a title made ENTIRELY of symbols folds to the empty string.
 * Real albums do this: Caravan Palace's "<|°_°|>", Ed Sheeran's "+", anything
 * by "!!!". keyFor() returned null for them, and null meant the album could
 * never be favourited at all: has() answered false so the heart painted hollow
 * forever, and toggle() bailed with `return false`, which the client read as
 * the NEW state and reported as "Removed from Favourites" — a failure wearing
 * the wording of the opposite success.
 */
function symFold(s) {
  return String(s || "").toLowerCase()
    .normalize("NFKD")
...
/* NB the fallback is on the TITLE ONLY, deliberately. The title is what gated
 * null, so applying it there can only rescue albums that previously had no key
 * and therefore no stored row — nothing existing can be orphaned. Doing the
 * same on the artist WOULD move the key of an already-stored favourite by a
 * symbol-named artist (a favourite of anything by "!!!" is filed under
 * "title|" today), stranding it. Residual, and accepted: two symbol-titled
 * albums by two symbol-named artists collide. That is strictly better than
 * neither being favouritable.
 */
function keyFor(title, artist) {
  const t = norm(title) || symFold(title), a = norm(artist);
...
function factory(opts) {
  ...
  function add(album) {
    ...
    const rec = {
      ...
      // The URL this album was filed under in LMS's OWN Favorites, when the
      // mirror succeeded. Stored so removal deletes exactly what was added
      // rather than a string rebuilt later from a title that may since have
      // been edited — LMS matches favourites by exact url.
      lms_url:   (album && album.lms_url) || null,
...
module.exports.symFold = symFold;
