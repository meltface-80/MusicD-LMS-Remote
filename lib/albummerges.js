  ...
  function apply(rows) {
    ...
    for (const m of merges) {
      ...
      for (const p of missing) {
        ...
        if (row && rowMatch.has(row)) row = null;            // already claimed
        if (!row) {
          // Title-only, and ONLY when exactly one album in the whole library
          // carries that title. Two albums sharing a title (a "Greatest Hits"
          // by different artists) is precisely when this must not guess.
          const cands = (titleIndex.get(norm(p.origTitle || p.title)) || []).filter(r => !rowMatch.has(r));
