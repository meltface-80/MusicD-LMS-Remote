  ...
  for (let i = 0; i < code.length; i++) {
    if (code[i] === "{") {
      ...
      if (f) {
        ...
        // A block whose body holds no nested block is a DECLARATION block; an
        // at-rule wrapper (@media …) is not. Walking at every depth rather than
        // only the top level is load-bearing — the album hero lives inside an
        // `@media (max-width: 719px)`, and a top-level-only walk cannot see it.
        if (!/[{}]/.test(body) && !f.prelude.startsWith("@")) {
          if (f.prelude.split(",").map((x) => x.trim()).includes(want)) out.push(body);
        }
      }
      start = i + 1;
    }
...
// ---------------------------------------------------------------------------
// 5b. THE ALBUM HERO'S CORNER BUTTONS (v1.0.85).
// ---------------------------------------------------------------------------
// Back, Share and Edit sit ON the artwork on the album view, and it can be any
// colour at all. The glass chip they wear everywhere else is a 7% wash — it
// vanishes into half the covers in a library. They take a DARK scrim there
// instead, in every palette, because white-on-dark is the only pair that
// survives an unknown sleeve; a theme-coloured button does not.
{
  const heroChrome = rules(".modal:not(.np-mode) .modal-close");
  if (!heroChrome.length) {
    bad("the album hero's chrome", "no scrim rule for .modal:not(.np-mode) .modal-close — the back"
      + " chevron would be a 7% wash on an unknown album cover");
  } else {
    const body = heroChrome.join("\n");
    const m = /background:\s*rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/.exec(body);
    const dark = m && (+m[1] + +m[2] + +m[3]) / 3 < 90;
    if (dark) ok("the album hero scrims its corner buttons dark");
    else bad("the album hero's chrome", "its scrim is not dark: " + (m ? m[0] : "no rgba background"));
    if (/#fff|255,\s*255,\s*255\)/.test(body)) ok("…and prints them white");
    else bad("the album hero's chrome", "the icon colour is not white");
    // Same rule as everywhere else: the artwork scrolls under these.
    if (/backdrop-filter/.test(body))
      bad("the album hero's chrome", "carries a backdrop-filter over scrolling artwork");
    else ok("…with no backdrop-filter over the scrolling artwork");
  }
  // The hero must be scoped OFF the tablet layout, where .modal-panel is a
  // centred 960px card: "full-bleed" there bleeds off the card's own padding,
  // which is a misalignment, not a bleed. Scoped by an explicit max-width query
  // rather than by source order, so reordering cannot switch it on.
  const heroSrc = code.slice(Math.max(0, code.indexOf(".modal:not(.np-mode) .modal-art") - 2000),
                             code.indexOf(".modal:not(.np-mode) .modal-art"));
  if (/@media\s*\(max-width:\s*719px\)\s*\{[^@]*$/.test(heroSrc))
    ok("the album hero is scoped to the phone by an explicit media query");
  else
    bad("the album hero", "is not inside an @media (max-width: 719px) block — it would apply to"
      + " the ≥720px centred-card layout, where a bleed is a misalignment");
}
