...
// A repair may only ever move the ARTIST half of a key — never the title.
// A full rescan renumbers ids AND re-detects artists, so the album now holding
// a part's OLD id is very often a completely unrelated record. Absorbing it
// would delete it from the library and rewrite the merge with its key.
{
  reset();
  const m = mk();
  m.merge([
    { title: "The Wall (Disc 1)", artist: "Pink Floyd", id: "101" },
    { title: "The Wall (Disc 2)", artist: "Pink Floyd", id: "102" },
  ]);
  const out = m.apply([
    { id: 501, title: "The Wall (Disc 1)", subtitle: "Various Artists" },
    { id: 502, title: "The Wall (Disc 2)", subtitle: "Various Artists" },
    { id: 101, title: "Kind of Blue",      subtitle: "Miles Davis" },
    { id: 102, title: "Abbey Road",        subtitle: "The Beatles" },
  ]);
  const titles = out.map(r => r.title);
  ok("a stale id pointing at an unrelated album is never absorbed",
     titles.includes("Kind of Blue") && titles.includes("Abbey Road"), titles);
  const wall = out.find(r => r.mergeId);
  ok("...and the merge repairs onto the RIGHT discs, by title",
     wall && wall.partIds.join(",") === "501,502", wall && wall.partIds);
  ok("...and the re-keyed parts keep their real titles",
     m.list()[0].parts.every(p => /^the wall disc/.test(p.key)), m.list()[0].parts.map(p => p.key));
}

// The title fallback must not absorb an unrelated album that merely shares a
// title, when nothing else about the merge is live.
{
  reset();
  const m = mk();
  m.merge([
    { title: "Live", artist: "Band A", id: "1" },
    { title: "Live (Disc 2)", artist: "Band A", id: "2" },
  ]);
  // Both original discs are gone from the library. One unrelated album happens
  // to be called "Live" — a lone title hit with a different artist and nothing
  // else matching is not evidence.
  const out = m.apply([{ id: 900, title: "Live", subtitle: "Someone Else" }]);
  ok("a lone title hit with a different artist is not absorbed",
     out.length === 1 && !out[0].mergeId, out.map(r => r.title + "/" + (r.mergeId || "-")));
}

// ...but when the WHOLE set moved artist together (the VA case), the title
// matches are believable and the set is repaired.
{
  reset();
  const m = mk();
  m.merge([
  ...
  const out = m.apply([
    { id: 700, title: "Sandinista!", subtitle: "Various Artists" },
    { id: 701, title: "Sandinista! Disc 2", subtitle: "Various Artists" },
  ]);
  ok("the whole set moving artist together IS repaired",
     out.length === 1 && out[0].partIds.join(",") === "700,701", out.map(r => r.partIds));
}
