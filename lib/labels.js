...
const { mbWait, MB_UA: MB_USER_AGENT } = require("./musicbrainz");
...
function makeLabels(deps = {}) {
  ...
  // Route through the leveled logger (lib/log.js). Messages already carry an
  // inline "[labels…]" tag from the Roon-era code; strip it so the logger's own
  // [labels] tag isn't doubled up.
  const lg = require("./log").makeLogger("labels");
  function log(msg) { lg.debug(String(msg).replace(/^\[labels(?::[^\]]*)?\]\s*/, "")); }
  ...
  function writeJson(file, obj) {
    ...
    } catch (e) { lg.debug("write failed:", file, e.message); }
  }
  ...
  let itunesLastBatch = 0, tadbLastReq = 0, discogsLastReq = 0, qobuzLastReq = 0;
  ...
  // mbWait comes from the SHARED gate in lib/musicbrainz.js — the other rate
  // limits below are per-service and stay local.
  ...
  async function fetchLabelFromiTunes(title, artist) {
    ...
    try {
      ...
      lg.child("itunes").debug(e.message);
      return null;
    }
  }
  ...
  async function fetchLabelFromMusicBrainz(title, artist) {
    ...
    } catch (e) { lg.child("mb").debug(e.message); }
    return null;
  }
  ...
  async function fetchLabelMbidFromMusicBrainz(labelName) {
    ...
    } catch (e) { lg.child("mb:label").debug(e.message); }
    return null;
  }
  ...
  async function fetchLabelFromTheAudioDB(title, artist) {
    ...
    try {
      ...
      if (!label || isLikelyNotALabel(label)) return null;
      return label;
    } catch (e) { lg.child("theaudiodb").debug(e.message); return null; }
  }
  ...
  async function fetchLabelFromDiscogs(title, artist) {
    ...
    try {
      ...
      if (!label || isLikelyNotALabel(label)) return null;
      return label;
    } catch (e) { lg.child("discogs").debug(e.message); return null; }
  }
  ...
  // cascade. This scrapes qobuz.com's PUBLIC album pages only — no account,
  // no API, unrelated to the (removed) Qobuz streaming-account integration.
  async function fetchLabelFromQobuz(title, artist) {
    ...
    try {
      ...
      return { label, year };
    } catch (e) { lg.child("qobuz").debug(e.message); return null; }
  }
  ...
  async function buildFileLabelMap(onProgress) {
    ...
    catch (e) { lg.child("files").debug("music-metadata not available:", e.message); return map; }
    ...
    catch (e) { lg.child("files").debug("scan error:", e.message); }
  ...
  async function runScan() {
    ...
    try {
      ...
      kickFanArtFetches().then(() => kickDiscogsLogoFetches()).catch(e => { lg.debug("logo fetch:", e.message); });
    } catch (e) {
  ...
  async function setLogo(label, url) {
    ...
    await assertPublicUrl(imageUrl);   // SSRF guard — reject private/loopback/link-local targets
    try {
      const ctl = new AbortController();
