...
function createLms(opts = {}) {
  ...
  async function qobuzBrowse(playerId, itemId, start = 0, count = 50) {
    ...
    const items = rows.map(it => {
      ...
      if (nodeId != null) return { kind: "node", item_id: nodeId, title: qLabel(it), icon: it.icon || null };
      return null;
    }).filter(Boolean);
    return { title: r.title || "", total: num(r.count) || items.length, start, items, notice: qobuzNotice(r) };
  }
