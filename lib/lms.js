...
function createLms(opts = {}) {
  ...
  function menuAction(base, item, which) {
    ...
    delete params.isContextMenu;      // a per-row menu hint, never wanted in a replayed play/add/go
  ...
  async function qobuzAlbumTracks(playerId, itemId) {
    ...
    const tracks = rows.map(it => {
      const label = qLabel(it);
      if (it.addAction === "go" || it.type === "link") return null;      // navigation node
      if (FAV_RE.test(label)) return null;                               // the favourite toggle
      if (/^(play|add|shuffle)\b.*\b(all|album)\b/i.test(label)) return null; // "Play all" shortcut
      const play = menuAction(base, it, "play");
      if (!play) return null;
      const { title, artist } = qobuzTitleArtist(it);
      return { title: title || label, artist, duration: num(it.duration) || null, play, add: menuAction(base, it, "add") };
    }).filter(Boolean);
