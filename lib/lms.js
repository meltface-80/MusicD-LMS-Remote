...
function createLms(opts = {}) {
  ...
  async function qobuzSearchNode(playerId) {
    ...
    try {
      ...
      const node = (r.item_loop || []).find(it =>
        it.type === "search" || /search/i.test(it.name || "") || /search/i.test(it.title || ""));
      _qobuzSearchNode = node ? node.id : null;
  ...
  async function qobuzSearchAlbums(playerId, term, count = 20) {
    ...
    if (items.length && !items.some(isAlbumRow)) {
      const cat = items.find(it => /album/i.test(it.name || it.title || "") && it.id != null);
      if (cat) {
        log.debug("qobuz search: descending into category", JSON.stringify(cat.name || cat.title), cat.id);
        const r2 = await request(playerId,
          ["qobuz", "items", 0, count, "item_id:" + cat.id, "menu:1"]);
        base = r2.base; items = r2.item_loop || [];
      } else {
        log.debug("qobuz search: no album rows and no album category — items were:",
          JSON.stringify(items.slice(0, 3).map(it => ({ name: it.name || it.title, type: it.type }))));
      }
    }
    const parsed = items.filter(isAlbumRow).map(it => {
      const line = String(it.name || it.title || "");                 // "Artist - Album" or "Album\nArtist"
      ...
      return {
        ...
        play:   menuAction(base, it, "play") || menuAction(base, it, "go"),
        add:    menuAction(base, it, "add"),
        go:     menuAction(base, it, "go"),          // descend into the album's menu (for favourite toggle)
      };
  ...
  async function qobuzAlbumFavoriteToggle(playerId, descend, want) {
    if (!playerId) throw new Error("player required");
    if (!descend || !Array.isArray(descend.cmd)) throw new Error("no album menu");
    const r = await request(playerId, [...descend.cmd, ...paramArgs(descend.params), "menu:1"]);
    const base = r.base, items = r.item_loop || [];
    const fav = items.find(it => FAV_RE.test(it.name || it.title || ""));
    if (!fav) throw new Error("no favourite action in album menu");
    const isFav = REMOVE_RE.test(fav.name || fav.title || "");
    ...
    await request(playerId, [...act.cmd, ...paramArgs(act.params)]);
    return !!want;
  }

  // Descend a menu tree collecting album-id → descend-action for every favourite
  // album found under the root "Favorites" node (one "Albums" level down when
  // present). Best-effort and defensive; returns [] on any failure.
  async function qobuzFavoriteAlbums(playerId) {
    if (!playerId) return [];
    try {
      const root = await request(playerId, ["qobuz", "items", 0, 50, "menu:1"]);
      const favNode = (root.item_loop || []).find(it =>
        FAV_RE.test(it.name || it.title || "") && !/playlist|track|artist/i.test(it.name || ""));
      if (!favNode) return [];
      let r = await request(playerId, ["qobuz", "items", 0, 200, "item_id:" + favNode.id, "menu:1"]);
      let items = r.item_loop || [];
      // Favorites usually splits into Albums/Artists/Tracks — descend into Albums.
      const albCat = items.find(it => /album/i.test(it.name || it.title || "") && it.id != null && !isAlbumRow(it));
      if (albCat) { r = await request(playerId, ["qobuz", "items", 0, 500, "item_id:" + albCat.id, "menu:1"]); items = r.item_loop || []; }
      const base = r.base;
      return items.filter(isAlbumRow).map(it => ({
        id: qobuzAlbumId(it),
        title: it.line1 || it.name || it.title || "",
        artist: it.line2 || "",
        go: menuAction(base, it, "go") || menuAction(base, it, "play"),
      })).filter(a => a.id);
    } catch (e) { return []; }
  }
  ...
  function qobuzAlbumId(it) {
    const cand = [it.id, it.album_id, it.params && it.params.item_id, it.url].map(v => v == null ? "" : String(v));
    for (const c of cand) {
