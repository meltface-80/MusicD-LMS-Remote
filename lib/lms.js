...
function createLms(opts = {}) {
  ...
  function menuAction(base, item, which) {
    ...
    return { cmd: a.cmd, params: { ...(a.params || {}), ...(itemParams || {}) } };
  }
  ...
  async function qobuzSearchAlbums(playerId, term, count = 20) {
    ...
    const parsed = items.filter(isAlbumRow).map(it => {
      const line = qLabel(it);                                         // "Artist - Album" or "Album\nArtist"
      const parts = line.split(/\s+[–-]\s+|\n/);                       // best-effort split
      return {
        title:  it.line1 || (parts.length > 1 ? parts.slice(1).join(" - ") : line),
        artist: it.line2 || (parts.length > 1 ? parts[0] : ""),
