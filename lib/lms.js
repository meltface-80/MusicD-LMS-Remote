...
function createLms(opts = {}) {
  ...
  async function qobuzAlbumTracks(playerId, itemId) {
    ...
    const isTrack = (it) => {
      ...
      if (it.input || it.type === "search") return false;             // input / account prompts
      if (FAV_RE.test(qLabel(it))) return false;
      // Require an EXPLICIT play indicator — never treat an arbitrary row with a
      // (base-inherited) play action as a track. This keeps account / preset /
      // info rows (e.g. under "My Weekly Q") out of the track list.
      const playable = it.goAction === "play" || it.style === "itemplay" || it.type === "audio" || it.type === "track";
      return playable && !!menuAction(base, it, "play");
    };
