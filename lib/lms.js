...
function createLms(opts = {}) {
  ...
  async function qobuzSearchAlbums(playerId, term, count = 20) {
    ...
    // Qobuz search can return category sub-menus (Albums / Artists / Tracks).
    // If nothing here is directly playable, descend into an album-ish category.
    if (items.length && !items.some(isAlbumRow)) {
      const cat = items.find(it => /album/i.test(qLabel(it)));
  ...
  async function qobuzFavoriteAlbums(playerId) {
    ...
    try {
      ...
      if (items.length && !items.some(isAlbumRow)) {
        const albCat = items.find(it => /album/i.test(qLabel(it)));
