 ...
 *
 * Three keyless, public reads. None of them touches LMS: Smart Picks costs the
 * server exactly nothing beyond the library scan it already runs. Ported
 * verbatim from the MusicD Remote Roon extension — there is nothing
 * Roon-specific in here, which is the whole reason it ports unchanged.
 *
...
// Overridable so an end-to-end test can point them at a local stub. There is
// no other way to exercise the build: these are live public services, and a
// test that actually called them would be slow, flaky and rude.
const LB_BASE = process.env.MDR_LB_BASE || "https://labs.api.listenbrainz.org/similar-artists/json";
const LB_STATS = process.env.MDR_LB_STATS || "https://api.listenbrainz.org/1/stats/sitewide/artists";
const MB_BASE = process.env.MDR_MB_BASE || "https://musicbrainz.org/ws/2/artist";
