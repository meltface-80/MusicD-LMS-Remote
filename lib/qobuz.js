...
// Single-album details — album/get. The interesting field is `description`
// (Qobuz's editorial/wiki-style album review, HTML-ish text — callers strip
// markup). Unsigned endpoint, same auth story as the rest of this client.
async function getAlbum(token, albumId) {
  if (!albumId) throw new Error("albumId required");
  const r = await qobuzGet("album/get", { album_id: albumId }, token);
...
module.exports = { login, getFeaturedAlbums, searchCatalog, getArtist, getAlbum, favoriteAlbum, unfavoriteAlbum, getFavoriteAlbumIds, pickImage, md5Hex, APP_ID };
