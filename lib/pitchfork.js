 ...
 */
const log = require("./log").makeLogger("pitchfork");
...
async function fetchPitchfork(title, artist) {
  ...
  try {
    ...
    // COMPLIANCE: log only the queried URL + status/message — NEVER review
    // text. A 404 means Pitchfork has no review at the guessed slug (the
    // normal, expected outcome for most albums), so it's a debug note, not an
    // error; anything else (timeout, network, rate-limit) is a real failure.
    if (/\b404\b/.test(e.message)) {
      log.debug("no review found (normal):", JSON.stringify(title), "—", JSON.stringify(primaryArtist), url);
    } else {
      log.warn("lookup failed for", JSON.stringify(title) + ":", url, "—", e.message);
    }
    pitchforkCache.set(key, null);
    return null;
  }
...
async function fetchPitchforkListing(path) {
  ...
  if (!raw) { log.debug("no preloaded state in", path); return []; }
  let state;
  try { state = JSON.parse(raw); }
  catch (e) { log.debug("state parse failed:", e.message); return []; }
