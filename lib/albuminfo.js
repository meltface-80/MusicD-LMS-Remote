 ...
 *
 * Source: the LMS "Music and Artist Information" plugin (musicartistinfo) via
 * JSON-RPC, when it is installed — probed with a `can` query and cached.
 * Lookups are done by LMS album_id/artist_id where we have one, so the plugin
 * can use the MusicBrainz ids it stores for tagged local files; name-based
 * lookup is the fallback for streaming/unmatched albums. The plugin aggregates
 * reviews/bios/photos from allmusic, last.fm, Wikipedia, etc., so it covers
 * both local and online-library albums. (Earlier builds also had a direct
 * Qobuz-API fallback; that was removed — the LMS Qobuz plugin already brings
 * online albums into the library, and MAI supplies their metadata.)
 *
...
const { mbWait, mbQuote, MB_UA } = require("./musicbrainz");
// Overridable so an end-to-end test can stub MusicBrainz — see lib/discovery.js.
const MB_ARTIST_BASE = process.env.MDR_MB_BASE || "https://musicbrainz.org/ws/2/artist";
...
// ---- tiny HTML → text (MAI descriptions/bios can be HTML-ish) --------------
 ...
 */
function makeAlbumInfo({ getLms, dataDir, normalize, artistKey, log, debug }) {
  log = log || require("./log").makeLogger("albuminfo");
  ...
  function scheduleSave() {
    ...
    saveTimer = setTimeout(() => {
      ...
      } catch (e) { log.debug("cache save failed:", e.message); }
    }, 3000);
  ...
  async function maiAlbumReview({ albumId, title, artist }) {
    ...
    for (const cmd of attempts) {
      ...
      } catch (e) { log.debug("mai albumreview:", e.message); }
    }
    return null;
  }
  ...
  async function maiBiography(name) {
    ...
    } catch (e) { log.debug("mai biography:", e.message); }
    return null;
  }
  ...
  // MusicBrainz etiquette (1 req/second, identified) is enforced by the SHARED
  // gate in lib/musicbrainz.js, not here: this module used to hold its own
  // timestamp, as did albumart and labels, so three of them could each believe
  // they had waited and fire in the same second.
  ...
  async function maiArtistPhotos(name, wantMany) {
    ...
    for (const cmd of cmds) {
      ...
      } catch (e) { log.debug("mai " + cmd[1] + ":", e.message); }
    }
    return [];
  }
  ...
  // An artist's MusicBrainz id, by name. Smart Picks needs one per seed artist
  // to ask ListenBrainz for neighbours, and it lives here because this module
  // already owns the MusicBrainz plumbing.
  //
  // MusicBrainz search is FUZZY and always returns something, so taking hit #1
  // blind hands back a different act entirely for short names (Low, Air, Ash,
  // Yes). The name has to actually match. artistKey folds stylisation, so
  // "P!nk" matches "Pink"; a name it canonicalises to nothing (a non-Latin
  // script) falls back to a raw comparison rather than rejecting every
  // candidate and losing the artist altogether.
  const artistMbidCache = new Map();
  async function artistMbid(name) {
    const nm = String(name || "").trim();
    if (!nm) return null;
    const key = artistKey(nm) || nm.toLowerCase();
    if (artistMbidCache.has(key)) return artistMbidCache.get(key);
    let mbid = null;
    try {
      await mbWait();
      const json = await httpJson(
        MB_ARTIST_BASE + "/?query=" +
          encodeURIComponent('artist:"' + mbQuote(nm) + '"') + "&fmt=json&limit=5",
        { "User-Agent": MB_UA }, 20000);
      const rows = (json && json.artists) || [];
      const raw = (x) => String(x || "").trim().toLowerCase();
      const want = artistKey(nm);
      const hit = (want && rows.find(a => a && artistKey(a.name) === want)) ||
                  rows.find(a => a && raw(a.name) === raw(nm)) ||
                  (!want ? rows[0] : null);
      mbid = hit && hit.id ? String(hit.id) : null;
      if (!hit) log.debug("mb artist: no name match for", JSON.stringify(nm),
                          "(top hit:", JSON.stringify(rows[0] && rows[0].name) + ")");
    } catch (e) { log.debug("mb artist:", e.message); }
  ...
  async function mbArtistMembers(name) {
    ...
    } catch (e) { log.debug("mb members:", e.message); }
    return empty;
  }
  ...
  // → { text, source: "LMS", attribution } | null
  ...
  // Photo source: MAI plugin (artistphotos → artistphoto). Membership: MusicBrainz "member of band" relations — for a band,
  ...
  async function artistInfo(name) {
    ...
    return dedupe("a:" + key, async () => {
      ...
      const photos = maiPhotos.slice(0, 4);
      const v = {
        name,
        photo:    photos[0] || null,
        photos,
  ...
  async function artistBio(name) {
    ...
    return dedupe("b:" + key, async () => {
      ...
      if (again !== undefined) return again;
      const v = await maiBiography(name);
      cachePut("bios", key, v);
      return v;
    });
  }

  return { albumReview, artistBio, artistInfo, pluginAvailable, artistMbid,
           _internal: { stripHtml, clampText, normalizePhotoUrl } };
}
