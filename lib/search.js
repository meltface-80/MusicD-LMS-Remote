...
// Numeric Qobuz album id from an LMS extid ("qobuz:album:123…") — lets the UI
// mark a library album that is a Qobuz favourite. null for anything else.
function qobuzIdFromExtid(extid) {
  const m = String(extid || "").match(/^qobuz:album:(\d+)/i);
  return m ? m[1] : null;
}
...
// Sort key for A-Z ordering. Distinct from normalize() (search matching) and
// artistKey() (identity folding): this one exists to make an alphabetical list
// read the way a record shelf does — case/accent folded, punctuation collapsed,
// and a leading English article dropped so "The Beatles" files under B and
// "A Love Supreme" under L. An album called just "The" keeps its own name
// rather than sorting as an empty string.
const LEADING_ARTICLE_RE = /^(?:the|a|an)\s+/;
function sortKey(s) {
  const n = normalize(s);
  return n.replace(LEADING_ARTICLE_RE, "").trim() || n;
}
...
function indexRecord(album) {
  ...
  return {
    ...
    offset:    album.offset,
    title, subtitle,
    // The original LMS title/artist when an owner edit renamed the album —
    // the artwork/edit stores key by this so they survive library rescans.
    origTitle:  album.origTitle  || title,
    origArtist: album.origArtist || subtitle,
    edited:     !!album.edited,
    year:      album.year != null ? album.year : null,
    image_key: album.coverId || null,
    // The LMS year/cover before any owner override — lets "Remove edits" fall
    // back to the real LMS values rather than the overrides.
    origYear:     album.origYear !== undefined ? album.origYear : (album.year != null ? album.year : null),
    origImageKey: album.origCoverId !== undefined ? album.origCoverId : (album.coverId || null),
    ...
    // Genre (first one, when LMS supplies it) — lets the Library genre facet
    // filter in memory instead of re-querying LMS per request.
    genre:     album.genre || null,
    // The URL LMS's own Favorites list files this album under, when the server
    // volunteered it (9.0 only). Carried through so a favourite can be mirrored
    // into LMS without reconstructing — and therefore re-escaping — the string.
    favUrl:    album.favUrl || null,
    // Epoch seconds this album was added to the library, filled in by a
    // background sweep over the track table (LMS has no album-level tag for
    // it). Absent until that lands, and for albums LMS reports no time for —
    // the "Date added" sort holds those out rather than treating them as 0.
    addedAt:   album.addedAt != null ? album.addedAt : null,
    // Multi-disc merge (lib/albummerges.js). `partIds` holds EVERY constituent
    // LMS album id in disc order — playback and the track list must walk all of
    // them, because LMS only ever understands one album_id at a time.
    mergeId:   album.mergeId || null,
    partIds:   Array.isArray(album.partIds) ? album.partIds.slice() : null,
    partCount: album.partCount || null,
    nTitle, nArtist,
    // Alphabetical-sort keys ("The Beatles" → "beatles"); see sortKey().
    sortTitle:  sortKey(title),
    sortArtist: sortKey(subtitle),
...
// Recompute a record's derived search fields in place after its title/subtitle
// changed (an owner edit). The byId/byOffset maps hold the same object, so no
// re-linking is needed — only the normalized/token/artist fields go stale.
function reindexRecord(index, rec) {
  const nTitle  = normalize(rec.title || "");
  const nArtist = normalize(rec.subtitle || "");
  rec.nTitle = nTitle;
  rec.nArtist = nArtist;
  rec.tTitle  = nTitle  ? nTitle.split(" ")  : [];
  rec.tArtist = nArtist ? nArtist.split(" ") : [];
  rec.jTitle  = nTitle.replace(/ /g, "");
  rec.jArtist = nArtist.replace(/ /g, "");
  // Sort keys are derived from title/subtitle too — an owner rename must
  // reorder the album in the Library's A-Z, not leave it filed under its old
  // name.
  rec.sortTitle  = sortKey(rec.title || "");
  rec.sortArtist = sortKey(rec.subtitle || "");
  rec.artistNames = splitArtistNames(rec.subtitle || "");
  rec.mainArtists = mainArtistNames(rec.subtitle || "");
  return rec;
}
...
function loadRecords(index, albumRows) {
  ...
  for (const r of index.records) {
    ...
    // A merged album must be reachable by EVERY part's LMS id, not just its
    // primary's — the genre-facet remap and the album-of-the-day pick both
    // resolve albums through byId and would otherwise lose absorbed discs.
    if (r.partIds) for (const pid of r.partIds) index.byId.set(String(pid), r);
  }
...
function searchAlbums(index, query, limit = 40) {
  ...
  return out.slice(0, limit).map(({ al, score }) => ({
    ...
    qobuz_id:  qobuzIdFromExtid(al.extid),
    score
  }));
}
...
module.exports = {
  normalize, artistKey, sortKey, splitArtistNames, mainArtistNames, indexRecord, reindexRecord, scoreAlbum,
  makeIndex, loadRecords, searchAlbums, searchArtists, qobuzIdFromExtid
};
