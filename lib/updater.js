...
function makeUpdater({ owner, repo, currentVersion, dir, viaLauncher, token, debug } = {}) {
  ...
  // matching tarball URL so apply() knows what to download:
  //   1. latest release (richest — notes + a canonical html_url + tarball_url)
  ...
  async function resolveLatest() {
    ...
    if (rel && rel.tag_name) {
      ...
      const dl = tarAsset ? tarAsset.browser_download_url : (rel.tarball_url || null);
    ...
    if (Array.isArray(tags) && tags.length && tags[0] && tags[0].name) {
      ...
      return {
        ...
        downloadUrl: apiRepo + "/tarball/" + encodeURIComponent(tag),
      };
    }
    ...
    if (pkgJson && typeof pkgJson.version === "string") {
      return {
        ...
        downloadUrl: apiRepo + "/tarball/main",
      };
    }
