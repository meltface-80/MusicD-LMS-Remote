...
function makeUpdater({ owner, repo, currentVersion, dir, viaLauncher, token, debug } = {}) {
  ...
  const lg = require("./log").makeLogger("updater");
  function log(msg) { lg.debug(msg); }
  ...
  async function ghFetch(url) {
    ...
    try {
      ...
      // A non-2xx is normal control flow here (this repo has no releases/tags,
      // so /releases/latest and /tags are *expected* to 404). resolveLatest()
      // narrates the fallback at debug; keep the raw status at trace so it
      // doesn't read as an error.
      if (!res.ok) { lg.trace(url + " -> HTTP " + res.status); return { _status: res.status }; }
      return await res.json();
    } catch (e) {
      lg.trace(url + " -> " + e.message);
      return null;
    } finally {
      clearTimeout(timer);
    }
  ...
  async function resolveLatest() {
    ...
    lg.debug("no published release; trying tags");
    ...
    // 3. package.json on the default branch (final fallback). This is the
    // NORMAL success path for this repo (no releases, no tags — see CLAUDE.md).
    lg.debug("no tags; resolving version from package.json on main (expected for this repo)");
    ...
    if (pkgJson && typeof pkgJson.version === "string") {
      lg.debug("latest resolved from package.json on main:", stripV(pkgJson.version));
      return {
    ...
    lg.warn("could not resolve a latest version from any GitHub source");
    return null;
  }
  ...
  async function apply() {
    ...
    try {
      ...
      // No shell: the args are already an array, and shell:true would break on an
      // install path containing a space and execute metacharacters in one.
