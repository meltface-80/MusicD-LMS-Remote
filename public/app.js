...
/* Installed-app viewport, MEASURED rather than trusted.
 *
 * THE FAILURE THIS EXISTS FOR: in an installed iOS PWA the LAYOUT viewport can
 * be taller than the area you can actually see. Every `position: fixed;
 * bottom: 0` box then lands BELOW the fold — the mini transport, the bottom of
 * the now-playing panel, the side menu's last row — and the strip along the
 * bottom of the display shows nothing. That is why no amount of padding INSIDE
 * those panels ever moved it, and why the layout measures correct in every
 * harness: the boxes are the right size, they are just positioned past the
 * edge of what the screen shows.
 *
 * `visualViewport` is the only thing that reports the visible area, so the gap
 * between it and `innerHeight` is published as `--app-gap` and the full-screen
 * containers end there instead of at `bottom: 0`. Where the two agree — every
 * desktop browser, every correct engine — the gap is 0px and this changes
 * precisely nothing.
 *
 * SCOPED TO THE INSTALLED APP on purpose. In a browser tab the same two numbers
 * diverge for an ordinary reason (retracting toolbars), `dvh` already handles
 * it, and shifting the UI there would undo the v1.0.56 side-menu fix.
 *
 * The KEYBOARD is the one other thing that shrinks the visual viewport, and it
 * must not be mistaken for this bug: with a field focused the gap is forced to
 * 0 so the app does not lurch upward while you type.
 */
(function () {
  ...
  function sync() {
    let gap = 0;
    if (vv && installed() && !typing()) {
      // Round DOWN so a sub-pixel rounding difference can never invent a gap.
      gap = Math.max(0, Math.floor(window.innerHeight - vv.height));
      // A gap larger than a third of the display is not this bug — treat it as
      // something transient (a rotation mid-flight) rather than shrink the app
      // to a sliver on the strength of one bad reading.
      if (gap > window.innerHeight / 3) gap = 0;
    }
    root.style.setProperty("--app-gap", gap + "px");
    root.style.setProperty("--app-vh", (window.innerHeight - gap) + "px");
  }
  sync();
  ...
  // Read by /diag.html, so the numbers behind a report are the app's own.
  window.__viewportGap = () => ({
    innerHeight: window.innerHeight,
    visual: vv ? Math.round(vv.height) : null,
    gap: parseInt(getComputedStyle(root).getPropertyValue("--app-gap"), 10) || 0,
    installed: installed(),
  });
})();
